...
\newcommand{\Challenge}{\ChallengeDie }
\newcommand{\abd}{\AbilityDie }
\newcommand{\bbb}{\BoostDie }
\newcommand{\Boost}{\BoostDie }
...
%    \item \advantage Was on the side of the Confederacy of Independent Systems during the Clone Wars. Reduced to vassal status after the Clone Wars and a source of fuel for the Imperial Military, Sullust became an essential mining and manufacturing center for the Empire. Sullustans don’t even count as citizens in the Empire.
    ...
    \item \Triumph Stan Tevv, former Galactic Senator, became SoroSuub CEO following the Battle of Yavin.  He has a secret agreement with the Rebel Alliance but officially claims neutrality.
    \item \Threat Alter a minor detail in one of the above extra facts
...
\begin{itemize}
    ...
    \item \Threat A detail that suggests heavier Imperial presence
\end{itemize}
...
Check:\\
Social checks to interact with Zien are Average [\df\df]. If PCs try \textbf{Coercion}, add a \stb, as she does not like being bossed around. If they attempt \textbf{Negotiation} in order to get payment for the rescue, that will earn a cinematic punch to the nose from her. 
...
Hard [\df\df\df] +\stb  \textbf{Perception}
\begin{itemize}
    \item \Success Each uncanceled \Success removes a \stb for difficult terrain in the upcoming \textbf{Piloting} check.
\end{itemize}
...
A successful check removes the \stb\stb\stb for difficult terrain from the \textbf{Piloting} check.
...
\df per Silhouette + \stb\stb\stb + Handling \textbf{Piloting}
...
\begin{itemize}
	...
	\item \Threat \Threat A lava eruption strikes the ship.  Suffer 2 system strain.
...
\df per Silhouette + \stb\stb + Handling \textbf{Piloting}
\begin{itemize}
    \item \Failure Hull trama
    \item \Threat System strain
	\item \Despair Vehicle critical hit
\end{itemize}
Check: Leads\\
Hard [\df\df\df] \textbf{Perception} or \textbf{Computers} +\stb to use the ships sensors to look for life signs (or use the Force). Doing this now will grant +\Boost on the \textbf{Survival} check to come.

\section{Underground Travel}

The PCs exit the ship into hot, sulfurous air, with ash and smoke blackening it. All PCs without a relevant Species Ability (like Twi'leks) or Talent (like Outdoorsman) add \stb due to the inhospitable environment.  If they lack filter masks, each PC suffers one wound ``per round'' (in structured play, else as narratively appropriate).\\
Check: Climb below the surface\\
Hard [\df\df\df] \textbf{Athletics} +\Boost for climbing gear
\begin{itemize}
  \item \Threat\Threat Accidentally unhook filter mask, suffering 1 wound that ignores soak
...
If two or more PCs failed their \textbf{Athletics} check for climbing, the party has taken too long to make it down the shaft and the excessive heat wears on them further. Every PC must make Easy [\df] \textbf{Resilience}. On a \Failure that PC now suffers Heat Exhaustion.  This is a critical injury that adds \stb on rolls until back in the cool confines of the ship. This condition can be removed with an Easy [\df] \textbf{Medicine} check.
...
Check:\\
Average [\df\df] \textbf{Coordination} to squeeze through a crack to get into a side passage. \Boost for narratively small PCs (all Silhouette 0 characters, humans with a slighter frame, for example), \stb for larger PCs (all Silhouette 2+ characters, Wookies, brawny humans, etc).

Drutash scarabs attack when the PCs are in the crack!  Depending on the number of PCs, there may be one in front and one behind.  Or possibly they wait to attack until some PCs are out but others are still in the crack.\\
Check:\\
...
\begin{itemize}
	...
	\item \Threat The miners are especially weakened and have to be carried out.
	\item \Threat\Threat The miners need immediate medical attention.
...
\begin{GenesysTable}{ll}
    Abilities & \\
       Tools & +2 dmg, Melee, Engaged, Crit 5\\
        & Improvised (+\Threat), Inferior (+\stb) \\
\end{GenesysTable}\\
}
...
\end{quoting}
(See \url{https://starwars.fandom.com/wiki/Rockrender} for a reference.)\\
Check:\\
...
\end{GenesysTable}
   ...
   The rockrender is really just passing through and will not attack anyone unless provoked.
...
\begin{GenesysTable}{ll}
    Abilities & \\
       Plausible Deniability 1 & Remove \stb from all \\
       & Coercion and Deception checks. \\
       Nobody's Fool 2 & Upgrade difficulty of \textbf{Charm}, \textbf{Coercion},\\
       & and \textbf{Deception} checks made against\\
       &this target twice.\\
\end{GenesysTable}
}
